...
bool linux_debmod_t::gen_library_events(int tid)
{
    ...
    && _read_memory(tid, r_debug_ea, &rd, sizeof(rd), false) == sizeof(rd) )
  {
    ...
    for ( link_map *ptr=rd.r_map; ptr != NULL; )
    {
      ...
      if ( map.l_addr == 0 && *name != '\0' )
      {
        // base address undefined - take it from /proc/<pid>/maps file
        if ( dll_maps.is_empty() )
          prepare_dll_mapping(&dll_maps);
        map.l_addr = dll_maps.get_dll_base(name);
      }
...
//--------------------------------------------------------------------------
bool linux_debmod_t::check_for_new_events(chk_signal_info_t *csi)
{
  while ( true )
  {
    ...
    ldeb("-------------------------------\n");
    log(get_thread(csi->pid), " => waitpid: %s\n", status_dstr(csi->status));
...
int linux_debmod_t::get_debug_event(debug_event_t *event, int timeout_ms)
{
  ...
  QASSERT(30057, ti->state != STOPPED || exited || WIFEXITED(status));
...
bool linux_debmod_t::handle_process_start(pid_t _pid, attach_mode_t attaching)
{
  ...
  if ( attaching == AMT_ATTACH_BROKEN )
    options = WNOHANG;
  waitpid(pid, &status, options); // (should succeed) consume SIGSTOP
...
//--------------------------------------------------------------------------
// store map entries for future using in case a dll has no base addr in link map
void linux_debmod_t::prepare_dll_mapping(dll_mapping_t *dll_maps)
{
  rewind(mapfp);
  mapfp_entry_t me;
  while ( read_mapping(&me) )
    dll_maps->store_dll(me.fname.c_str(), me.ea1);
}

//--------------------------------------------------------------------------
void linux_debmod_t::dll_mapping_t::store_dll(const char *dllname, ea_t baseEA)
{
  const char *shname = qbasename(dllname);
  addrmap_t::iterator p = addrs.find(shname);
  if ( p == addrs.end() || baseEA < p->second )
    addrs[shname] = baseEA;
}

//--------------------------------------------------------------------------
ea_t linux_debmod_t::dll_mapping_t::get_dll_base(const char *dllname)
{
  char base[QMAXPATH+1];
  qstrncpy(base, qbasename(dllname), sizeof(base));
  char *pext = strchr(base, '.');
  if ( pext != NULL )
    *pext = '\0';
  size_t baseln = strlen(base);

  for ( addrmap_t::iterator p = addrs.begin(); p != addrs.end(); ++p )
  {
    if ( strneq(p->first.c_str(), base, baseln) )
      return p->second;
  }
  return 0;
}
