...
class linux_debmod_t: public linuxbase_debmod_t
{
  ...
  // dll base addresses as read from /proc/<pid>/maps
  class dll_mapping_t
  {
    typedef std::map<qstring, ea_t> addrmap_t;
    addrmap_t addrs;
  public:
    void store_dll(const char *dllname, ea_t baseEA);
    ea_t get_dll_base(const char *dllname);
    bool is_empty() { return addrs.empty(); }
    void cleanup()  { return addrs.clear(); }
  };
  ...
  void prepare_dll_mapping(dll_mapping_t *dll_maps);
...
public:
  ...
  bool check_for_new_events(chk_signal_info_t *csi);
