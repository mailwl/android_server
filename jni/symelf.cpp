...
static int _load_elf_symbols(linput_t *li, symbol_visitor_t &sv)
{
  ...
  if ( dlt.is_valid() )
  {
    ...
    {
      int link = dlt.link;
      uint32 dynsym = sections.get_index(section_headers_t::wks_dynsym);
      if ( link == 0 && dynsym != 0 )
        link = sections.get(dynsym)->sh_link;
  ...
  uint32 interp = sections.get_index(section_headers_t::wks_interp);
  if ( interp
  ...
  {
    uint32 symtab = sections.get_index(section_headers_t::wks_symtab);
    uint32 dynsym = sections.get_index(section_headers_t::wks_dynsym);
    uint32 strtab = sections.get_index(section_headers_t::wks_strtab);
    uint32 dynstr = sections.get_index(section_headers_t::wks_dynstr);
    if ( symtab || dynsym )
    {
      // Loading symbols
      if ( symtab )
        code = load_symbols(reader, *sections.get(symtab), strtab, sv);
      if ( code == 0 && dynsym )
        code = load_symbols(reader, *sections.get(dynsym), dynstr, sv);
    }
    else if ( di.symtab.size )
    {
